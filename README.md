...
#### 다국어 지원 (KO / EN 토글)
- **언어 전환 버튼**: 모든 페이지 navbar 우측에 `EN` / `KR` 버튼 추가. `localStorage`에 선택 저장 → 페이지 이동·재방문 시에도 유지
- **홈 페이지 번역**: About 소개문, Career 3개 회사 불릿 포인트 전체, Latest Posts·Guestbook UI 텍스트 한/영 분기
- **블로그 목록 번역**: 사이드바 설명, 검색 placeholder, 결과 카운트, 빈 상태 메시지 + 포스트 제목·excerpt 자동 번역
- **포스트 상세 번역**: 제목, 본문(마크다운) 전체 자동 번역. TOC도 번역된 헤딩 기준으로 재생성
- **Google Translate 비공식 API**: `server/api/translate.post.ts` 서버 라우트로 프록시 — 4000자 단위 청킹, 코드 블록 보호(번역 제외) 후 복원
- **번역 캐시**: 동일 텍스트 재번역 방지 (메모리 캐시)
- **로딩 표시**: 번역 중 pulse 애니메이션 인디케이터 표시
